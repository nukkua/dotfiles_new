...
const INSERT_CURSOR_SHAPE = "\x1b[1 q";
...
export class ModalEditor extends CustomEditor {
  private mode: Mode | "visual" = "insert";
  ...
  private visualLineAnchor: number | null = null;
  ...
  getMode(): Mode | "visual" { return this.mode; }
  ...
  handleInput(data: string): void {
    ...
    if (this.mode === "insert") {
      // Enter/Ctrl+j: insert internal newline without submitting prompt
      if (this.isEnterLikeInput(data) || matchesKey(data, "ctrl+j") || data === "\x0a") {
        super.handleInput(NEWLINE);
        return;
      }
        // Ctrl+h: backspace
      if (matchesKey(data, "ctrl+h") || data === "\x08") {
  ...
  private handleNormalMode(data: string): void {
    ...
    if (data === "V") {
      this.mode = "visual";
      this.visualLineAnchor = this.getCursor().line;
      return;
    }
  ...
  private handleVisualLineMode(data: string): void {
    ...
    if (data === "d" || data === "D") {
      const anchor = this.visualLineAnchor ?? this.getCursor().line;
      const cursorLine = this.getCursor().line;
      this.mode = "normal";
      this.visualLineAnchor = null;
      this.deleteLineRange(anchor, cursorLine);
      return;
    }
    ...
    if (data === "y" || data === "Y") {
      const anchor = this.visualLineAnchor ?? this.getCursor().line;
      const cursorLine = this.getCursor().line;
      this.yankLineRange(anchor, cursorLine);
      this.mode = "normal";
      this.visualLineAnchor = null;
      return;
    }
  ...
  private highlightVisualLineSelection(renderedLines: string[]): void {
    if (this.mode !== "visual") return;
    if (this.visualLineAnchor === null) return;

    const cursorLine = this.getCursor().line;
    const { start, end } = this.getNormalizedLineRange(this.visualLineAnchor, cursorLine);
    const renderedCursorLine = renderedLines.findIndex((line) => line.includes(CURSOR_MARKER));
    if (renderedCursorLine === -1) return;

    const firstRenderedLine = renderedCursorLine + (start - cursorLine);
    const lastRenderedLine = renderedCursorLine + (end - cursorLine);
    const from = Math.max(0, Math.min(firstRenderedLine, lastRenderedLine));
    const to = Math.min(renderedLines.length - 1, Math.max(firstRenderedLine, lastRenderedLine));

    for (let i = from; i <= to; i++) {
      const line = renderedLines[i];
      if (line === undefined) continue;
      renderedLines[i] = `${SOFTWARE_CURSOR_START}${line || " "}\x1b[27m`;
    }
  }
  ...
  render(width: number): string[] {
    ...
    this.highlightVisualLineSelection(lines);
  ...
  private getModeLabel(): string {
    ...
    if (this.mode === "visual") return " VISUAL LINE ";
